...
writer = None  #SummaryWriter('logs/')
from data_loader import cifar10_dataset_loader, cifar100_dataset_loader

sys.path.append("..")
from util import save_image_results, show_running_time
import runtime
import sgd
import adamw

parser = argparse.ArgumentParser(description='PyTorch CIFAR Training')
parser.add_argument('--log_dir', type=str, help='path to tensorboard log dir')
...
parser.add_argument('--no_input_pipelining',
                    action='store_true',
...
parser.add_argument('--checkpoint_dir_not_nfs',
                    action='store_true',
...
parser.add_argument('--num_ranks_in_server',
                    default=1,
                    type=int,
...
parser.add_argument('--square', action='store_true', help='s ** 2')

parser.add_argument('--world_size',
                    type=str,
                    help='world_size',
                    default=os.getenv('SLURM_NTASKS'))
parser.add_argument('--port', default=12345, type=int, help="port")

parser.add_argument('--partitions', type=int, default=1, metavar='P',
...
def main():
    ...
    if args.optim == 'sgd':
        optimizer = sgd.SGDWithPipeOptim(
            r.modules(),
            r.master_parameters,
            r.model_parameters,
            args.loss_scale,
    ...
    if args.optim == 'adamw':
        optimizer = adamw.ADAMWWithPipeOptim(
            r.modules(),
            r.master_parameters,
            r.model_parameters,
            args.loss_scale,
    ...
    train_loader, val_loader = cifar100_dataset_loader(args)
    # if checkpoint is loaded, start by running validation
    if args.resume:
        assert args.start_epoch > 0
        validate(val_loader, r, args.start_epoch - 1)

    runtime_list, val_top1_list, val_top5_list, val_loss_list = [], [], [], []
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.forward_only:
            ...
            time_end = time.time()
            runtime_list.append(time_end - time_start)

            show_running_time(epoch, args.lr, runtime_list[epoch])
            val_prec1, val_prec5, val_loss = validate(val_loader, r, epoch)
            val_top1_list.append(val_prec1)
            val_top5_list.append(val_prec5)
            val_loss_list.append(val_loss)

            print("maximum memory allocated:", args.rank, torch.cuda.max_memory_allocated())
            print("memory allocated:", args.rank, torch.cuda.memory_allocated())

            if is_last_stage():
                filename = 'PipeOptim_cifar100' + '_' + args.arch + '_' + args.optim + '_seed' + str(args.manualSeed)
                save_image_results(filename, runtime_list[1:], val_loss_list, val_top1_list, val_top5_list)
...
def train(train_loader, r, optimizer, epoch):
    ...
    print(f'optim_name = {optimizer.optim_name}')
    # start num_warmup_minibatches forward passes
    for i in range(num_warmup_minibatches):
        optimizer.load_predicted_weights(forward=True)  # for adamw
        r.run_forward()
    ...
    for i in range(n - num_warmup_minibatches):
        ...
        # optimizer.sgd_load_predicted_weights(forward=False)
        r.run_backward()
        # optimizer.load_old_params()

        # logging.info(f'Finish backward')
        #logging.warning(f'outside: {dist.get_rank()}: {args.find_median}, s = {s}')
        # if args.spectrain:
        #     # logging.info(
        #     #     f'main.py: line 627 | optimizer.step: {optimizer.step}')
        #     optimizer.step(s=s, find_median=args.find_median)
        # else:
        optimizer.step()
        # logging.info('After optimizer.step()')
        # inconsistent
        # optimizer.load_old_params()
        # r.run_backward()
    ...
    with torch.no_grad():
        ...
        for i in range(n - num_warmup_minibatches):
            ...
            if is_last_stage():
                ...
                prec1, prec5 = accuracy(output, target, topk=(1, 5))
                losses.update(loss.item(), output.size(0))
                top1.update(prec1[0].item(), output.size(0))
                top5.update(prec5[0].item(), output.size(0))
        ...
        if is_last_stage():
            print(' * Prec@1 {top1.avg:.2f}% Prec@5 {top5.avg:.2f}%'.format(
                    top1=top1, top5=top5))
...
def adjust_learning_rate(optimizer, epoch, total_epochs, r, lr_policy, step,
                         epoch_length):
    ...
    # if total_epochs < 50:
    #    total_epochs = 100
        #logging.warning('Total epochs set to 100')
    stage_base_lr = r.get_adjusted_learning_rate(base_lr=args.lr)

    if args.lr_warmup and epoch < 5:
        lr = stage_base_lr * float(1 + step +
                                   epoch * epoch_length) / (5. * epoch_length)

    else:
        if lr_policy == "step":
            lr = stage_base_lr * (0.1**(epoch // 90))
            ...
            decay_rate = 0.97
            lr = stage_base_lr * (decay_rate
                                  **(float(epoch) / float(total_epochs)))
        else:
            raise NotImplementedError

    # if step == 0:
    #    logging.info("Epoch: %d Step %d \tLearning rate: %f" %
    #                 (epoch, step, lr))
