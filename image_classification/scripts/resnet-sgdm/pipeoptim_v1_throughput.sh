...
python main_with_runtime_throughput.py --module models.resnet101.gpus=4 -b $BS --data_dir $dataset --config_path models/resnet101/gpus=4/mp_conf.json --distributed_backend gloo --master_addr $MASTER_ADDR --epochs ${EPOCH} --lr $LR --weight-decay $DECAY --print-freq 200 --log_dir logs/xpipe_${NUM_GPU}_lr${LR} --lr_policy step --rank 3 --local_rank 3 --world_size 4 \
 >> ./experiment_results/xpipe-p1-resnet101_tinyimagenet_throughput_bs86.txt
