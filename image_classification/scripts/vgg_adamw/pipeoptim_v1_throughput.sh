...
python main_with_runtime_cifar_throughput.py --optim adamw --module models.vgg16.gpus=4_cifar -b $BS --data_dir $dataset --config_path models/vgg16/gpus=4_cifar/mp_conf.json --distributed_backend gloo --master_addr $MASTER_ADDR --epochs ${EPOCH} --lr $LR --weight-decay $DECAY --print-freq 200 --log_dir logs/xpipe_${NUM_GPU}_lr${LR} --lr_policy step  --rank 3 --local_rank 3 --world_size 4 \
 >> ./experiment_results/pipeoptim-p1-cifar10-vgg16_adamw_throughput_bs2800.txt
