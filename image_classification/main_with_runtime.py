...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.forward_only:
            ...
            if is_last_stage():
                filename = 'PipeOptim_cifar10' + '_' + args.arch + '_' + "SGD" + '_seed' + str(args.manualSeed)
...
def train(train_loader, r, optimizer, epoch):
    ...
    for i in range(num_warmup_minibatches):
        optimizer.sgd_load_predicted_weights(forward=True)
        r.run_forward()
...
def adjust_learning_rate(optimizer, epoch, args):
    ...
    if epoch == 40 or epoch == 60:
        lr = lr / 10
        print("imagenet lr", lr)
    args.lr = lr

    for param_group in optimizer.param_groups:
        param_group['lr'] = lr

def adjust_learning_rate1(optimizer, epoch, total_epochs, r, lr_policy, step,
                         epoch_length):
