...
def save_translation_results(model_name, runtime_list):
    ...
    filename = ab_path + 'translation/final_result/' + model_name + \
...
def get_absolute_path():
    path = "/gf3/home/lei/project/pipedream-pipedream/runtime/"
    return path
...
def main():
    ...
    optimizer = adam.ADAMWithSpectrainCHC(
        r.modules(),
        r.master_parameters,
        r.model_parameters,
        args.loss_scale,
    ...
    if use_adam_optimizer:
        optimizer = optim.AdamWithWeightStashing(
        ...
        optimizer = optim.SGDWithWeightStashing(
            modules=r.modules(), master_parameters=r.master_parameters,
            model_parameters=r.model_parameters, loss_scale=args.loss_scale,
    ...
    runtime_list = np.zeros(args.epochs + 1)

    runtime_list, val_loss_list, train_loss_list = [], [], []
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.forward_only:
            ...
            train(train_loader, r, optimizer, epoch, train_loss_list, args.batch_size)
            time_end = time.time()
            runtime_list.append(time_end - time_start)
            show_running_time(epoch, args.lr, runtime_list[epoch])

            # evaluate on validation set
            prec1, val_loss = validate(val_loader, r, epoch)
            val_loss_list.append(val_loss)
            print("acc", args.rank, prec1, val_loss)
...
def train(train_loader, r, optimizer, epoch, train_loss_list, batch_size):
    ...
    for i in range(num_warmup_minibatches):
        optimizer.load_predicted_weights(forward=True)
        r.run_forward()
    ...
    for i in range(n - num_warmup_minibatches):
        ...
        # optimizer.load_predicted_weights(forward=False)
        r.run_backward()
        #optimizer.load_old_params()
        optimizer.step()

        if (i+1) == iteration_num - num_warmup_minibatches:
            print("iter", iteration_num, batch_size)
            end_time = time.time()
            elapsed_time = end_time - epoch_start_time
            throughput = batch_size * iteration_num / elapsed_time
            if is_last_stage():
                print("elapsed time", elapsed_time, "throughput (tok/s)", throughput)
            break
